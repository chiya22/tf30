...
function my_script_init()
{
    ...
    wp_enqueue_style('self', get_template_directory_uri() . '/css/self.css', array(), '1.0.2', 'all');
    wp_enqueue_script('my', get_template_directory_uri() . '/js/script.js', array('jquery'), '1.0.0', true);
    wp_enqueue_script('highlightjs', get_template_directory_uri() . '/js/highlight.pack.js', array('jquery'), '1.0.0', true);
    if (is_single()) {
...
function my_the_post_category($anchor = true, $id = 0)
{
    ...
    $this_post_categories = get_the_category($id);

    if ($this_post_categories[0]) {
        if ($anchor) {
            echo '<a href="' . esc_url(get_category_link($this_post_categories[0]->term_id)) . '">' . esc_html($this_post_categories[0]->cat_name) . '</a>';
        } else {
            echo esc_html($this_post_categories[0]->cat_name);
        }
    }
}
 ...
 * ハイライトのショートコード
 *
 * @param array $atts ハイライトの引数.
 * @param string $content ハイライトのコンテンツ.
 * @return string ハイライト定義のHTMLタグ
 */
 function my_highlight($atts, $content = '')
{
    return '<pre><code class="' . $atts['type'] . '">' . esc_html($content) . '</code></pre>';
}
